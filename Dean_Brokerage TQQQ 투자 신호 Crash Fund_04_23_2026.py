...
# ─────────────────────────────────────────────
# COST_BASIS: Dean_Brokerage_state.json에서 읽기
# ─────────────────────────────────────────────
STATE_FILE = "Dean_Brokerage_state.json"
DEFAULT_COST_BASIS = 60.00
...
def load_cost_basis():
    if os.path.exists(STATE_FILE):
        try:
            ...
            print(f"✅ Dean_Brokerage_state.json 로드: COST_BASIS = ${cb} (최종 업데이트: {state.get('last_updated','unknown')})")
            return cb
        except Exception as e:
            print(f"⚠️  Dean_Brokerage_state.json 읽기 오류 ({e}) → 기본값 ${DEFAULT_COST_BASIS} 사용")
    else:
        print(f"⚠️  Dean_Brokerage_state.json 없음 → 기본값 ${DEFAULT_COST_BASIS} 사용")
