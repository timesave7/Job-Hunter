"""
Dean's TQQQ Daily Signal Checker v3.2
=======================================
배당 Crash Fund 전략 기준 버전
GitHub Actions 자동 실행 + 이메일 발송
v3.1 변경사항: COST_BASIS를 state.json에서 읽기 + 전환 신호 이메일에 업데이트 버튼 추가
v3.2 변경사항: MAX_DIV_PCT 30% → 40% / Crash Hunter L1/L2 → L1/L2/L3 체계로 개편
"""
...
# ─────────────────────────────────────────────
# COST_BASIS: state.json에서 읽기 (없으면 기본값 사용)
# ─────────────────────────────────────────────
STATE_FILE = "Roth IRA_state.json"
DEFAULT_COST_BASIS = 43.50
...
def load_cost_basis():
    if os.path.exists(STATE_FILE):
        try:
            ...
            print(f"✅ state.json 로드: COST_BASIS = ${cb} (최종 업데이트: {state.get('last_updated','unknown')})")
            return cb
        except Exception as e:
            print(f"⚠️  state.json 읽기 오류 ({e}) → 기본값 ${DEFAULT_COST_BASIS} 사용")
    else:
        print(f"⚠️  state.json 없음 → 기본값 ${DEFAULT_COST_BASIS} 사용")
...
MAX_DIV_PCT   = 0.40   # 탄약풀 최대 비중 40% (현금+SCHD 합산)

# Crash Hunter 재매수 기준 (L1/L2/L3)
CRASH_1_PCT   = 0.45   # ATH 대비 -55% → 탄약풀 40% 재투입
CRASH_2_PCT   = 0.30   # ATH 대비 -70% → 탄약풀 40% 재투입
CRASH_3_PCT   = 0.20   # ATH 대비 -80% → 탄약풀 잔여 20% 전액 재투입

# GitHub Actions 업데이트 링크
GITHUB_REPO        = "timesave7/Dean-Automation"
UPDATE_WORKFLOW    = "Roth IRA_update_cost_basis.yml"
...
def build_signal_html(s):
    ...
    if s['crash_3_hit']:
        ...
        action_lines = [
            ...
            "→ 탄약풀 잔여 20% 전액 → TQQQ 매수 (현금 우선, 부족 시 SCHD 매도)",
        ]
        conv_warning_html = ""
        update_button_html = ""
        ...
        action_lines = [
            f"TQQQ ATH 대비 {s['tqqq_drop_pct']:.1f}% 폭락",
            "→ 탄약풀(현금+SCHD) 40% → TQQQ 매수 (현금 우선, 부족 시 SCHD 매도)",
        ]
        conv_warning_html = ""
        update_button_html = ""
        ...
        <div style="background:#fffde7;border-left:4px solid #f9a825;padding:12px 16px;
            ...
            ⚠️ <strong>전환 전 확인:</strong> 현재 탄약풀(현금+SCHD) 비중이 포트폴리오의
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;
    ...
    <div style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:white;
        ...
        <h1 style="margin:0;font-size:20px;">📊 Roth IRA TQQQ 투자 신호</h1>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:18px;">
            ...
            <tr style="border-bottom:1px solid #eee;">
                ...
                <td style="padding:8px;text-align:right;
                    ...
                    {'🚨 도달! 탄약풀 40% 투입' if s['crash_1_hit'] else '미달 ✅'}
                </td>
            </tr>
            <tr style="border-bottom:1px solid #eee;">
                ...
                <td style="padding:8px;text-align:right;
                    ...
                    {'🚨 도달! 탄약풀 40% 투입' if s['crash_2_hit'] else '미달 ✅'}
                </td>
            </tr>
            <tr>
                ...
                <td style="padding:8px;text-align:right;
                    ...
                    {'🚨 도달! 탄약풀 잔여 20% 투입' if s['crash_3_hit'] else '미달 ✅'}
                </td>
            </tr>
        </table>
...
def print_signal(s):
    ...
    print(f"     L1(-55%): ${s['crash_1_price']:.2f}  →  {'🚨 도달! 탄약풀 40% 투입' if s['crash_1_hit'] else '미달 ✅'}")
    print(f"     L2(-70%): ${s['crash_2_price']:.2f}  →  {'🚨 도달! 탄약풀 40% 투입' if s['crash_2_hit'] else '미달 ✅'}")
    print(f"     L3(-80%): ${s['crash_3_price']:.2f}  →  {'🚨 도달! 탄약풀 잔여 20% 투입' if s['crash_3_hit'] else '미달 ✅'}")
    ...
    if s['crash_3_hit']:
        ...
        print(f"       → 탄약풀 잔여 20% 전액 → TQQQ 매수 (현금 우선, 부족 시 SCHD 매도)")
    elif s['crash_2_hit']:
        print(f"\n  🚨  Crash Hunter L2 발동! (ATH 대비 {s['tqqq_drop_pct']:.1f}%)")
        print(f"       → 탄약풀(현금+SCHD) 40% → TQQQ 매수 (현금 우선, 부족 시 SCHD 매도)")
    elif s['crash_1_hit']:
...
def send_email(html_content, s):
    ...
    subject = f"[Roth IRA TQQQ투자신호] {signal} | TQQQ ${s['tqqq_price']} | {s['date']}"
